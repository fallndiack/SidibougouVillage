...
<div class="calendar">
    ...
    <div class="d-flex flex-row align-items-center justify-content-between mx-sm-3">
        ...
        <?php if (isset($_GET['delete'])) : ?>
            <div class="alert alert-success">
                L'évenement a été supprimé avec succés !!!
            </div>
        <?php endif ?>
        <div>
            ...
            <tr>
                ...
                ?>
                    <td class="<?= $month->withinMonth($date) ? '' : 'calendar__othermonth'; ?> <?= $isToday ? 'is-today' : ''; ?>">
                        ...
                        <?php endif; ?>
                        <a class="calendar__day" href="<?= $router->url('admin_calendrier_add') ?>?date=<?= $date->format('Y-m-d'); ?>"><?= $date->format('d'); ?></a>
                        <?php foreach ($eventsForDay as $event) : ?>
                            <div class="calendar__event">
                                <?= (new DateTime($event['start']))->format('H:i') ?> - <a href="<?= $router->url('admin_calendrier_edit') ?>?id=<?= $event['id']; ?>"><?= h($event['name']); ?></a>
                            </div>
                        <?php endforeach; ?>
                    </td>
                <?php endforeach; ?>
    ...
    <a href="<?= $router->url('admin_calendrier_add') ?>" class="calendar__button">+</a>
